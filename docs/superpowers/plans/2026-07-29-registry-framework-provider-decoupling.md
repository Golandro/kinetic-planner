...
**Architecture:** A generic `KPRegistry<T>` type-safe registry (main sourceSet, freezeable) replaces scattered static Maps and switch statements. `ProviderConfigBinding` (main interface) with a single generic client implementation `ModConfigSpecConfigBinding` encapsulates per-provider TOML read/write logic, eliminating the 4 switch branches in KPConfig. `MapProviderFactory` (client interface) replaces the static `new XaeroMapOverlayProvider()` block in MapOverlayDispatcher with a registry-driven approach. `ProviderConfigRegistry` is migrated to use `KPRegistry` internally, unifying the registry pattern.

> **Audit optimizations applied (2026-07-29):**
> - **O1 (high):** Merged `XaeroProviderConfigBinding` + `JourneyMapProviderConfigBinding` into single `ModConfigSpecConfigBinding` (eliminates >90% code duplication).
> - **O2 (medium):** `ProviderConfigRegistry` internal storage migrated to `KPRegistry<ProviderConfig>` (unifies registry pattern).
> - **O3 (low):** `MapOverlayDispatcher.initProviders` gains package-private test seam.
...
| `src/client/java/net/jsmua/kinetic_planner/config/ModConfigSpecConfigBinding.java` | Generic per-provider config binding (reads/writes TOML values via ModConfigSpec holders) |
...
| `src/main/java/net/jsmua/kinetic_planner/data/ProviderConfigRegistry.java` | Migrate internal storage to KPRegistry<ProviderConfig> (O2) |
...
### Task 4: ModConfigSpecConfigBinding - Generic Provider Config Binding

> **Audit optimization O1:** Merged original Task 4 (XaeroProviderConfigBinding) and Task 5 (JourneyMapProviderConfigBinding) into a single generic class to eliminate >90% code duplication.

**Files:**
- Create: `src/client/java/net/jsmua/kinetic_planner/config/ModConfigSpecConfigBinding.java`

This is a client sourceSet class - it captures `ModConfigSpec` value holders from `KPConfig`. A single generic implementation serves all providers; modId and displayName are injected at construction. No separate test needed because the logic is trivial delegation to ModConfigSpec (which requires NeoForge runtime to test); the binding's contract is verified in Task 3's `ProviderConfigBindingTest`.
...
/**
 * Generic {@link ProviderConfigBinding} backed by {@link ModConfigSpec} value holders.
 *
...
```bash
git add src/client/java/net/jsmua/kinetic_planner/config/ModConfigSpecConfigBinding.java
git commit -m "feat: add ModConfigSpecConfigBinding generic provider config binding"
```
...
```java
    ...
    /**
     * Test seam: inject provider instances directly, bypassing factory creation.
     *
     ...
     *
     * @param injected provider instances to use
     */
...
### Task 12.5: Migrate ProviderConfigRegistry to KPRegistry

> **Audit optimization O2:** Unify registry pattern by migrating ProviderConfigRegistry's internal storage from a raw `LinkedHashMap` to `KPRegistry<ProviderConfig>`.

**Files:**
- Modify: `src/main/java/net/jsmua/kinetic_planner/data/ProviderConfigRegistry.java`

- [ ] **Step 1: Replace internal storage and update all methods**
 ...
 *
 * <p><b>Note:</b> 内部 {@link KPRegistry} 实例不冻结（provider 默认配置可能在
...
```bash
git add src/main/java/net/jsmua/kinetic_planner/data/ProviderConfigRegistry.java
git commit -m "refactor: migrate ProviderConfigRegistry internals to KPRegistry"
```
