...
- 这表示当前编码工具没有正确应用项目通过`.agents/`目录管理的Agent配置。
- 须第一时间**阻断**当前任务，首先向用户提出关联`.agents/`目录到品牌工具目录的请求。
- 回复须包含：当前情况、通过编码工具加载规则的重要性、不分发品牌工具目录的理由、如何操作（最好的免维护方案是符号链接）。
...
2. **查阅 `STATUS.md`**：当前阶段状态、兼容性矩阵、技术假设核实。
3. **查阅实现计划与设计规格**：`docs/superpowers/plans/` 和 `docs/superpowers/specs/` 下的文档。
4. **搜索代码库**：使用搜索工具验证 API 签名、字段可见性等。如果可能，应该优先使用 LSP。
5. **搜索外部文档**：项目使用的外部依赖，其签名和定义文档需要通过 `Context7` 等工具获取，或者使用网络搜索。 
